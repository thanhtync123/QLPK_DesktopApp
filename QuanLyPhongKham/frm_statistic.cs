...
namespace QuanLyPhongKham
{
    ...
    WHERE updated_at BETWEEN '{fromDate}' AND '{toDate}'";
            ...
            string query2 = $@"
            SELECT sum(price) as total FROM `examinations` where type='toa thuốc' and created_at BETWEEN '{fromDate}' AND '{toDate}'
                            ";
            label_2.Text = string.Format("{0:N0} VND", Db.Scalar(query2));

            // Doanh thu dịch vụ
            string query3 = $@" SELECT sum(price) as total FROM `examinations` where type='chỉ định' and created_at BETWEEN '{fromDate}' AND '{toDate}'";
